...
proceeding from right to left in time. Pulse $P_i$ is applied at time $\blue{t_i={} }t_{i-1}+\tau_i$ where $t_{i-1}$ is the time of the application of $P_{i-1}$, and $t_0$ is the start time of the sequence. \rsout{(Please edit Fig.~1 to illustrate the sentences above, including indicating the time intervals; remove $Q_i$s from the figure. No need to mention gates here.)}
...
\red{(That is the infinite norm, i.e., the largest eigenvalue, and consistently used across the orginal script. The reason, which was explaned in the error phase section in version 1, is that the norm need to satisty $\norm{I}=1$ for whatever dimensionality of the identity. ---Jiaan)}
