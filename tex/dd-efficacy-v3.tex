...
\caption{\red{(Which one is for PDD? Why do some points violate the threshold condition? Is it because of numerical---sampling?---inaccuracies?) } \blue{(I removed the CDD figure for now. No, they did not violate the threshold condition. The condition says ``any point below the red line is gauranteed to remove noise'', which is true according to the graph. The points that introduce more noise after DD are not colored in the graph. Some colored points lie above the red line because they are randomly generated and there is nothing to do with inaccuracies.  )}The noise threshold compared for PDD \rsout{and $\mathrm{CDD}_3$} simulated at $\alpha=0$. The red solid line represents the noise threshold prediction $\eta\le\beta/4$. different colors represents different levels of noise reduction ration $\beta_\sDD/\beta$ after numerically maximized over random bath operator instances.
